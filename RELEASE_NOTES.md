...
- 进一步修复状态栏「显示主窗口」在特定时机仍无响应的问题：NSStatusItem 菜单项 action 在菜单 tracking runloop mode 下执行，`makeKeyAndOrderFront` 会被随后菜单关闭的动作吞掉；改为 `DispatchQueue.main.async` 将窗口激活延迟到菜单关闭后的下一个 runloop 周期执行。
